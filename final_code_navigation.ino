...
// Define I2C pins for I2C_0 and I2C_1
#define I2C0_SDA 21
#define I2C0_SCL 22
#define I2C1_SDA 27
#define I2C1_SCL 19
...
bool ended_super_star_task = false;
...
float kp2 = 1.3;
*/
float right_radius = 44.925;//45.0;
float left_radius = 44.825;//45.0;
//float kbm =70.0;
//float kbr = 0.0;
...
//  esp_task_wdt_init(30, false); // Disable watchdog for Core 1
  esp_task_wdt_delete(NULL); // Disable WDT for this task

    for(;;) {
      /*
        Wire.begin(I2C0_SDA, I2C0_SCL);
        int val1 = sensor.readRangeSingleMillimeters();
        Wire.end();
 ...
 /*
  if(jack==1){
    wakt_tawa=millis();
    wakt_tawa=wakt_tawa-wakt_debut_match;
    if(86000<wakt_tawa<100000){
      -sequance
      -test_capteur
      }
  }
...
void moveMotors(void *parameter) {
    esp_task_wdt_delete(NULL); // Disable WDT for this task

    for(;;) {
//while ( ended_super_star_task == false) {
...
analogWrite(2, 150);
Robot_Locate(700,0);
Robot_Locate(1100,-350);
Robot_Locate(1700,-350);
activate_sensor=false;
Robot_Locate(2000,-100);
...
*/
//startegy lel west
delay(85010); // zone west bemgikya 
analogWrite(2, 150);
activate_sensor=false;
Robot_Locate(250,0);
Robot_Locate(800,-250);
//activate_sensor=false;
Robot_Locate(1500,-170);
Robot_Locate(1500,-100);
...
activate_sensor=false;
Robot_Locate(250,0);
Robot_Locate(800,250);//kenet (800,250)
//activate_sensor=false;
Robot_Locate(1500,170);//kenet (1500,250)
Robot_Locate(1500,100);
...
void rotate(double goal) {
  ...
  target_a = goal * PI * spacing_encoder / 360;   //regle 3  2*pi*r--->360
  first = now;
  while (((fabs(target_a - totalR) > 2) || (fabs(target_a + totalL) > 2)) ) {  //(fabs(target_a-totalR)>2) || (fabs(target_a+totalL)>2)
   ...
   // pos_error = target_a - (GyroAngle - salpha) / 180 * PI * spacing_encoder / 2;  //(alpha-salpha)*spacing_encoder/2;
    /*
     pos_error=target_a -(totalR-totalL)/2;
    d_error = (pos_error - prev_pos_error);
    prev_pos_error = pos_error;
    consigne_vel = kp1r * pos_error + kd1r * d_error;
    */
    Setpointd_a = target_a;
...
*/
      ...
      if (millis()<=(init_wakt+100000)) {
       ...
       // Serial.println("ena mestopi angle");
        while(1==1){
        stop_motors();
        }
        /*
         ...
         Outputl_a=0;
         Outputd_a=0;
         myPID_a.SetMode(MANUAL);
         myPID_L_a.SetMode(MANUAL);
         myPID_distance_a.SetMode(MANUAL);
         */
   ...
   // k++;
    /*
      if (k==10){
      Serial.print("target_a_angel:");
      Serial.print(Setpointd_a);
      Serial.print(",");
      Serial.print("target_a_velocity==PD_output:");
...
//((Outputl*200)/1000)
      ...
      //Outputl=constraint((Outputl+ (kbm * bal_error)),-500,500);
       ...
       if ((stopMotors==false)&& (millis()<=(init_wakt+100000))){
